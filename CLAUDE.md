...
- `internal/providers` — the engine. This is where almost all real work happens. `dump.go` is the portable dump: `ExportDump`/`ImportDump`/`SyncDump` implement the one dump format (manifest.json + mirrored stores + instructions/memory) and the rclone push/pull.
