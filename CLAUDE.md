...
- `internal/providers` — the engine. This is where almost all real work happens. `dump.go` is the portable dump: `ExportDump`/`ImportDump`/`SyncDump` implement the one dump format (manifest.json + mirrored stores + instructions/memory) and the rclone push/pull. `convexport.go` / `transcript.go` are the conversation export: `ExportConversations` writes one `qa.jsonl` of full Q&A plus tools/results (`sessionId`, `folder`, `q`, `a`, `time`, `messages`) with no agent, model, or token fields, via parallel reads and an atomic rename.
