...
Run a single test: `go test ./pkg/crossmem -run TestName`. Library tests live in `pkg/crossmem/*_test.go` (`client_test.go` builds isolated `Client`s over SQLite/JSONL fixtures — prefer it to env-driven tests); CLI tests in `internal/app`. `go run ./examples/list <folder>` exercises the library against this machine's real stores.
...
2. **SQLite** (Devin, OpenCode, Copilot CLI) — `listDevin`/`listOpenCode`/`listCopilotCLI` open the DB read-only and query directly. With a folder filter they must **not** `LIMIT` in SQL — the folder is matched in Go (`sameOrChild`, case-folded on Windows), so rows stream newest-first until `limit` matches (`listQuery`). OpenCode subagent sessions (`parent_id` set) are hidden by default and surfaced as `Session.Children`; `ListOptions.IncludeSubagents` / `list --subagents` lists them with `Session.Parent`. Devin uses one `sessions.db` for every session (CLI and desktop alike — the desktop drives the CLI through an ACP connector); OpenCode uses `~/.local/share/opencode/opencode*.db` (`session`/`message`/`part` tables, `session.directory` = cwd); the GitHub Copilot **CLI** (distinct from the VS Code Copilot JSONL store) uses `~/.copilot/session-store.db` (`sessions.cwd` + a denormalized `turns` table). Each exposes sessions via a `Ref` (`devin:<id>` / `opencode:<id>` / `copilot-cli:<id>`) that `resolveRef` routes on. For a file Ref, the provider comes from the client's own store roots first (`providerForPath`), so every Ref `List` returns loads — including stores relocated by `$CLAUDE_CONFIG_DIR` or a config override. The sibling `auth.json` / `auth.db` / credential tables are never read.
...
`config.go` layers user overrides from `~/.config/crossmemcli/config.json` (or `$CROSSMEM_CONFIG`) on top: `stores` replaces a store's candidates, `extraStores` appends. Keys are `provider:kind`, or a bare `provider` which resolves only to the definition marked `Primary` — so overriding `devin` cannot repoint `devin:cli-logs` at a database file. Unknown keys are a load error: `New` returns it, `crossmem config` surfaces it, and the CLI's default client logs it and runs on the built-ins. The config is held by the `Client`, not cached process-wide; tests either build a `Client` with an explicit `Config` or repoint `$CROSSMEM_CONFIG` and call `resetConfig`.
